...
if (!defined('BASEPATH'))
    ...
    public function ValidateApiKey()
    {
    	...
    	if(!empty($header['X-Api-Key']) && $header['X-Api-Key']=='soor')
    	{
    		return TRUE;
    	}
    	else
    ...
    public function ThrowError($msg,$rstatus,$status)
    {
    	$body=array('message'=>$msg,'rstatus'=>$rstatus,'status'=>$status);
		echo json_encode($body);
    }

    public function SendOkResponse($msg,$rstatus,$status,$data)
    {
    	$body=array('message'=>$msg,'rstatus'=>$rstatus,'status'=>$status,'data'=>$data);
		echo json_encode($body);
    }
	...
	{				
		...
		{			
			...
			$this->SendOkResponse('OK',1,200,$category);
		}
		else $this->ThrowError('Unautorized user access.',0,401);
	}
	...
	public function MasterItemList()
	{
    	if($this->ValidateApiKey())
		{
        	...
        	$this->SendOkResponse('OK',1,200,$vals);				
		}
		else $this->ThrowError('Unautorized user access.',0,401);
	}
	...
	public function AddToCart()
	{
		...
		{
			$input= $this->ReadInput();			
        	$vals=$this->user_api_model->add_to_cart($input);
        	$this->SendOkResponse('OK',1,200,$vals);				
		}
		else $this->ThrowError('Unautorized user access.',0,401);
	}
	...
	public function UserRegistration()
    {
		...
		{
			$input= $this->ReadInput();
			$query = $this->db->get_where('tbl_users', array('email' => $input['email']));
			$row = $query->row_array();			
			if(empty($row))
			{
				...
				{	
					...
					$this->SendOkResponse('OK',1,200,$rows);
				}
				else $this->ThrowError('Opps ! maybe this mobile number is already registered with us.',0,401);
			}
			else $this->ThrowError('User Already Registered.',0,401);
		}
		else $this->ThrowError('Unautorized user access.',0,401);
	}
	...
	public function UserLogin()
	{
    	if($this->ValidateApiKey())
		{
	        ...
	        if(!empty($row))
	        {
	        	$uid=$this->user_api_model->get_user_id_from_emailid($input['email']);
	        	$device_id=$this->user_api_model->get_device_id_from_user_id($uid['userId']);
	        	if($device_id['tbl_users_device_id']==$input['tbl_users_device_id'])
	        	{
	        		$up=$this->user_api_model->updte_user_device_details($uid['userId'],$device_id['tbl_users_device_id'],$input);	        		
	        	}
	        	else
	        	{
	        		$ad=$this->user_api_model->users_device_add($input,$uid['userId']);
	        	}
	        	...
	        	$vals=$this->user_api_model->user_get($uid['userId'],$device_id['tbl_users_device_id']);
	        	//echo "<pre>";print_r($uid);die;
	        	if(!empty($vals))
				{
					$this->SendOkResponse('OK',1,200,$vals);
				}
				else $this->ThrowError('something went wrong please tyr after some time.',0,401);
			}
			else $this->ThrowError('Invalid user name of password please retry with right credentials.',0,401);
		}
		else $this->ThrowError('Unautorized user access.',0,401);
	}
	...
	public function UserAddress()
	{
		if($this->ValidateApiKey())
		{
			...
			if($input['function']==='add')
			{
				...
				$this->SendOkResponse('Address Added.',1,200,array());
			}
			...
			if($input['function']=='delete')
			{
				...
				$this->SendOkResponse($result.' Record Deleted.',1,200,array());
			}
		}
		else $this->ThrowError('Unautorized user access.',0,401);
	}
	...
	public function UserAddressList()
	{
		if($this->ValidateApiKey())
		{
			...
			else $this->SendOkResponse('No Records Found.',1,200,array());
		}
		else $this->ThrowError('Unautorized user access.',0,401);
	}
	...
	public function UpdateUserPassword()
	{
		if($this->ValidateApiKey())
		{
        	...
        	if(!empty($vals))
			{
				...
				$this->SendOkResponse('Password Succesfully Updated.',1,200,array());
			}
			else $this->SendOkResponse('Invalid Current Password.',0,200,array());
		}
		else $this->ThrowError('Unautorized user access.',0,401);
	}
	...
	public function UserPlaceOrder()
	{
		...
		{
			$input= $this->ReadInput();
			if(!empty($input['cart_boi']) && $input['payment_method']=='COD')
			{
				$grand_total=0.0;
				foreach ($input['cart_boi'] as $keys => $values)
                {
                	$grand_total+=($values['item_master_price']*$values['item_qty']);
                }
				$order_ref_no=$this->user_api_model->place_order($input,$grand_total,$payment_method=0);
				$val=$this->user_api_model->get_order_detail_by_ref_no($order_ref_no);
        		$val['order_items']=$this->user_api_model->get_order_items_by_ref_no($order_ref_no);
        		$this->SendOkResponse('Order Succesfully created.',1,200,$val);
			}
   ...
   //      	if(!empty($vals))
	...
	public function UserOrderList()
	{
		...
		{
			$input= $this->ReadInput();
			$val=$this->user_api_model->get_user_order_list($input);
        	if(!empty($val))
			{
				$body=array('message'=>'OK','rstatus'=>1,'status'=>200,'data'=>$val);
				echo json_encode($body);
			}
			else
			{
