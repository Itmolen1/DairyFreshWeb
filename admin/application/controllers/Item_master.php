...
class Item_master extends BaseController
{
            ...
            if(isset($_FILES) && $_FILES['item_master_logo']['error']==0)
            {
                ...
                if($imageFileType == "jpg" || $imageFileType == "png" || $imageFileType == "jpeg")
                {
                    if ( move_uploaded_file($_FILES['item_master_logo']['tmp_name'],$tfile))
                    {
                        ...
                        $this->resizeImage($uid.'.'.$ex);
                    }                        
                }
                /*image upload*/
            }

            $result = $this->item_master_model->add_new_item_master($value);
    ...
    {
        $source_path = $_SERVER['DOCUMENT_ROOT'] . '/admin/assets/item/' . $filename;
        $target_path = $_SERVER['DOCUMENT_ROOT'] . '/admin/assets/item/thumbnail/';
        $config_manip = array(
          'image_library' => 'gd2',
