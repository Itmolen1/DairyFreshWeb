<?php
	...
	class User_api_model extends CI_Model 
	{
		...
		{
			$this->db->select('item_category_id,item_category_name,item_category_logo');
		...
		{
			if(isset($val['cart_ref_no']) && $val['cart_ref_no']!='')
			{
				$query=$this->db->get_where('tbl_cart_boi',array('cart_ref_no'=>$val['cart_ref_no'],'item_master_id'=>$val['item_master_id']));
				$result=$query->row_array();
				if(empty($result))
				{
					// add new item in boi for existing cart user 
					$cdata=array(
			        	'cart_id'=>$val['cart_id'],
			        	'cart_ref_no'=>$val['cart_ref_no'],
			        	...
			        	'item_total'=>($val['item_master_price']*$val['item_qty'])
			        );
			        $this->db->insert('tbl_cart_boi', $cdata);
				}

				else
				{
					$this->db->set('item_master_price',$val['item_master_price']);
					$this->db->set('item_qty',$val['item_qty']);
					$this->db->set('item_unit_id',$val['item_unit_id']);
					$this->db->set('item_total',($val['item_qty']*$val['item_master_price']));
					$this->db->where('cart_id',$val['cart_id']);
					$this->db->where('cart_ref_no',$val['cart_ref_no']);
					$this->db->where('item_master_id',$val['item_master_id']);
					$this->db->update('tbl_cart_boi');
				}

				$sql = "SELECT SUM(item_total) as item_total FROM tbl_cart_boi WHERE cart_ref_no = '".$val['cart_ref_no']."'";
		        ...
		        $this->db->set('cart_subtotal',$subtotal);
		        $this->db->set('cart_discount',$discount);
		        $this->db->set('cart_grand_total',$grand_total);
		        $this->db->where('cart_id',$val['cart_id']);
		        $this->db->update('tbl_cart');

				$this->db->select('cart_id,cart_ref_no,cart_user_id,cart_subtotal,cart_discount,cart_grand_total');
		        $this->db->where('cart_ref_no',$val['cart_ref_no']);
		        $query=$this->db->get('tbl_cart');
		        $result=$query->row_array();

		        $this->db->select('cart_boi_id,cart_id,item_master_id,item_unit_id,item_master_price,item_qty,item_total');
		        $this->db->where('cart_ref_no',$result['cart_ref_no']);
		        $query=$this->db->get('tbl_cart_boi');
		        $boi=$query->result_array();
		        $result['cart_items']=$boi;
		        return $result;
			}
			else
			{
				// first time user and first item 
				$query=$this->db->get_where('tbl_cart',array('cart_user_id'=>$val['cart_user_id']));
				$result=$query->row_array();
				if(empty($result))
				{
					$refno=md5(microtime().uniqid());
					$data=array(
				        'cart_user_id'=>$val['cart_user_id'],
				        'cart_ref_no'=>$refno,
				        'cart_created_at'=>date('Y-m-d H:i:s'),
			        ...
			        $subtotal=$val['item_master_price']*$val['item_qty'];
			        $discount=0;
			        $grand_total=$subtotal-$discount;
			        $this->db->set('cart_subtotal',$subtotal);
			        $this->db->set('cart_discount',$discount);
			        $this->db->set('cart_grand_total',$grand_total);
			        $this->db->where('cart_id',$lid);
			        $this->db->update('tbl_cart');
		...
		public function user_add($data)
		{
			...
			$this->db->insert('tbl_users', $value); 
			$lid=$this->db->insert_id();
			return $lid;
		}

		public function users_device_add($data,$userId)
		{
			$value=array(
				'userId'=>$userId,
				'roleId'=>3,
				'tbl_users_device_id'=>$data['tbl_users_device_id'],
				'tbl_users_platform'=>$data['tbl_users_platform'],
				'tbl_users_device_token'=>$data['tbl_users_device_token'],
		...
		function get_user_id_from_emailid($emailid)
		{
			...
			$query=$this->db->get_where('tbl_users',array('email'=>$emailid));
			$row = $query->row_array();
			return $row;
		}
		...
		{
			$this->db->select('tbl_users_devices_id');
			$query=$this->db->get_where('tbl_users_devices',array('userId'=>$uid));
			$row = $query->row_array();
			return $row;
		}
		...
		{
			$this->db->set('tbl_users_device_id',$data['tbl_users_device_id']);
			$this->db->set('tbl_users_platform',$data['tbl_users_platform']);
			$this->db->set('tbl_users_device_token',$data['tbl_users_device_token']);
			$this->db->where('userId', $uid);
			$this->db->where('tbl_users_device_id', $device_id);
		...
		public function user_get($userId,$devid=null)
		{
			...
			if($devid!=null)
			{
				...
				$query=$this->db->get_where('tbl_users_devices',array('userId'=>$userId,'tbl_users_device_id'=>$devid));
				$row = $query->row_array();
            	//return $row;
			}
			//echo "<pre>";print_r($row);die;
			$this->db->select('userId,name,mobile,email,tbl_user_image');
			$query = $this->db->get_where('tbl_users', array('userId' => $userId));
		...
		public function add_user_address($val)
		{
			$data=array(
		        'userId'=>$val['userId'],
		        'user_address_type'=>0,
		...
		public function update_user_address_by_id($data)
		{
			$this->db->set('userId',$data['userId']);
			$this->db->set('user_address_type',0);
			$this->db->set('user_address',$data['user_address']);
			$this->db->set('user_latitude',$data['user_latitude']);
			$this->db->set('user_longitude',$data['user_longitude']);
			$this->db->set('person_name',$data['person_name']);
			$this->db->set('person_mobile',$data['person_mobile']);
			$this->db->set('city',$data['city']);
			$this->db->set('state',$data['state']);
			$this->db->set('pin',$data['pin']);
			$this->db->set('updated_at',date('Y-m-d H:i:s'));
			$this->db->where('user_address_id', $data['user_address_id']);
			$this->db->update('tbl_user_address');
			return $this->db->affected_rows();
		}

		public function delete_user_address($data)
		{
			$this->db->where('user_address_id', $data['user_address_id']);
		...
		function get_user_address_list($data)
		{
			...
			// $sql="SELECT user_address_id,user_address,user_latitude,user_longitude,person_name,person_mobile FROM tbl_user_address  WHERE isDeleted = 0 ORDER BY user_address_id DESC LIMIT ".$offset.",".$limit."";
			$sql="SELECT user_address_id,user_address_type,user_address,user_latitude,user_longitude,person_name,person_mobile FROM tbl_user_address  WHERE isDeleted = 0 AND userId = ".$data['userId']." ORDER BY user_address_id DESC ";
			$query = $this->db->query($sql);				
		...
		function get_user_password($data)
		{
			...
			$query = $this->db->get_where('tbl_users', array('userId' => $data['userId'],'password'=>$data['password_old']));
			$row = $query->row_array();
			//echo $this->db->last_query();die;
            return $row;
		}
		...
		function update_user_password($data)
		{
			...
			$this->db->where('password', $data['password_old']);
		...
		function place_order($input,$grand_total,$payment_method)
	    {
	    	...
	    	for($i=0;$i<count($input['cart_boi']);$i++)
	    	{
		    	$cdata=array(
					'order_id'=>$lid,
					'order_ref_no'=>$order_ref_no,
					'item_master_id'=>$input['cart_boi'][$i]['item_master_id'],
					'item_master_name'=>$input['cart_boi'][$i]['item_master_name'],
					'item_master_url_slug'=>$input['cart_boi'][$i]['item_master_url_slug'],
					'item_master_unit'=>$input['cart_boi'][$i]['item_master_unit'],
					'item_master_price'=>$input['cart_boi'][$i]['item_master_price'],
					'item_master_logo'=>$input['cart_boi'][$i]['item_master_logo'],
					'item_qty'=>$input['cart_boi'][$i]['item_qty'],
					'created_at'=>date('Y-m-d h:i:s'),
					'updated_at'=>date('Y-m-d h:i:s'),
					);
	    ...
	    {
	    	$this->db->select('BaseTbl.order_id,BaseTbl.order_ref_no,BaseTbl.userId,BaseTbl.payment_method,BaseTbl.payment_status,BaseTbl.status,BaseTbl.order_grand_total,ad.user_address_id,ad.user_address');
	    ...
	    function get_user_order_list($data)
		{
			$offset=$data['page_num'];
			$limit=$data['page_size'];
			$offset=$offset-1;
			...
			if($data['userId']!='')
			{
				$sql="SELECT o.order_id,o.order_ref_no,o.userId,o.user_address_id,o.payment_method,o.payment_status,o.status,o.order_grand_total FROM tbl_order o LEFT JOIN tbl_user_address ad ON ad.user_address_id = o.user_address_id WHERE o.userId = ".$data['userId']." ORDER BY o.order_id DESC LIMIT ".$offset.",".$limit."";
				$query = $this->db->query($sql);				
				$rows=$query->result_array();
		...
		function get_uris()
		{
			$query=$this->db->get('tbl_app_control');
			return $query->result_array();
		}

		public function insert_feedback($val)
		{
			$data=array(
		        'user_id'=>$val['user_id'],
		        'message'=>$val['message'],
		...
		public function insert_sr_docs($val,$tfile,$thumb)
		{
			$data=array(
		        'sr_id'=>$val['sr_id'],
		...
		public function get_emp_all_docs_by_sr_id($sr_id,$sr_docs_type)
		{
			$this->db->select('sr_docs_id,sr_id,is_image,sr_docs_url,sr_docs_thumb');
		...
		{
			$this->db->select('sr_docs_id,sr_id,sr_docs_url,sr_docs_thumb');
			$query=$this->db->get_where('sr_docs',array('sr_id'=>$sr_id,'sr_docs_type'=>$sr_docs_type));
			$result=$query->result_array();
			return $result;
		}
		...
		function check_admin_single_employee_reports_data($input)
		{
			if(isset($input['report_type']) && $input['report_type']=='employee_report')
			{
				$this->db->select('sr.*,user.tbl_user_name,emp.tbl_employee_name');//,,sm.sales_master_grand_total
				$this->db->from('tbl_service_request as sr');
				$this->db->where('sr.isDeleted',0);
				$this->db->where('sr.assigned_emp_id',$input['tbl_employee_id']);
				if(isset($input['status']) && $input['status']!='all')
				{
					//echo "coming";die;
					$this->db->where('sr.status',$input['status']);
		...
		function get_details_for_sq($srid)
		{
			if(!empty($srid))
			{				
				$this->db->select('sr.sr_id,sr.tbl_user_id,sr.assigned_emp_id,u.tbl_user_name,e.tbl_employee_name,sr.service_request_ref');
				$this->db->from('tbl_service_request as sr');
		...
		function get_emp_dev_detail_by_sr_id($sr_id)
		{
			$this->db->select('assigned_emp_id');
			$query=$this->db->get_where('tbl_service_request',array('sr_id'=>$sr_id));
			$empid=$query->row_array();
			$this->db->select('tbl_emp_device_type,tbl_emp_device_token');
			$query=$this->db->get_where('tbl_employee',array('tbl_employee_id'=>$empid['assigned_emp_id']));
			$dev=$query->row_array();
			return $dev;
		}
		...
		function update_sq_pdf_link($sqid,$url,$sr_id,$grand_total)
		{
			...
			if($caller=='UpdateSalesQuotation')
			{
				$res[]=array('status_name'=>'Quotation Updated','status_time'=>$ctime);
			}		
			else
			{
		...
		{
			if(!isset($data['sr_latitude']))
			{
				$data['sr_latitude']=$data['tbl_user_latitude'];
				$data['sr_longitude']=$data['tbl_user_longitude'];
				$data['sr_place']=$data['tbl_user_place'];
			}
			...
			$value=array(
				...
				'tbl_user_latitude'=>$data['tbl_user_latitude'],
				'tbl_user_longitude'=>$data['tbl_user_longitude'],
				'tbl_user_place'=>$data['tbl_user_place'],
				'sr_latitude'=>$data['sr_latitude'],
				'sr_longitude'=>$data['sr_longitude'],
				'sr_place'=>$data['sr_place'],
				'status'=>0,
				'isNew'=>1,
				'created_at'=>date('Y-m-d H:i:s'),
				'updated_at'=>date('Y-m-d H:i:s'),
				'service_request_ref'=>$refno,
				'status_history'=>json_encode(array(array('status_name'=>'Order Placed','status_time'=>date('Y-m-d H:i:s'))))
				);
		...
		public function newschedule_service($data)
		{
			$refno=$this->get_refrence_no();
			$value=array(
				'tbl_user_id'=>$data['tbl_user_id'],
		...
		public function service_history($user_id,$limit,$offset)
		{
			$offset=$offset-1;
			$offset=$offset*$limit;
		    $sql = "SELECT t.sr_id,t.service_request_ref,t.prefferd_date,ts.time_slot_name,t.status,t.isQuotation,t.pending_amount FROM tbl_service_request t  LEFT JOIN tbl_time_slot ts ON t.prefferd_time = ts.time_slot_id WHERE t.tbl_user_id = ".$user_id." ORDER BY t.sr_id DESC LIMIT ".$offset.",".$limit."";
		    //echo $sql;die;
			$query = $this->db->query($sql);
			$row=$query->result_array();
		...
		function my_task_list($empid)
		{
			...
			$this->db->where('sr.status !=', 11);
			//$this->db->where('sr.status !=',12);
		...
		function emp_ordre_detail($sr_id)
		{
			...
			{
				$this->db->select('sales_quotation_id,sales_quotation_pdf');
				$this->db->where('sales_quotation_sr_id', $sr_id);
				$query1 = $this->db->get('tbl_sales_quotation');
				$row1 = $query1->row_array();
		...
		function user_ordre_detail($sr_id)
		{
			...
			if($row)
			{
				...
				$this->db->select('sr_docs_id,is_image,sr_docs_url,sr_docs_thumb');
				$this->db->from('sr_docs');
				$this->db->where('sr_id',$row['sr_id']);
				$this->db->where('sr_docs_type',0);
				$query=$this->db->get();
				$result=$query->result_array();
				$row['before_service_doc']=$result;

				$this->db->select('sr_docs_id,is_image,sr_docs_url,sr_docs_thumb');
				$this->db->from('sr_docs');
				$this->db->where('sr_id',$row['sr_id']);
				$this->db->where('sr_docs_type',1);
				$query=$this->db->get();
				$result=$query->result_array();
				$row['after_service_doc']=$result;			
			}
			//echo $this->db->last_query();die;
			//echo "<pre>dsf";print_r($row);die;
			if($row['isQuotation']==2)
		...
		function confirm_sales_quotation($sr_id)
		{
			...
			$res=json_decode($res['status_history'],true);
			$ctime=date('Y-m-d H:i:s');
			$res[]=array('status_name'=>'Quotation Sent by Employee','status_time'=>$ctime);
			$res=json_encode($res);
		...
		function user_confirm_sales_quotation($sr_id,$isconfirm)
		{
			$this->db->select('status_history');
			$this->db->from('tbl_service_request');
			$this->db->where('sr_id',$sr_id);
			...
			$res=json_decode($res['status_history'],true);
			$ctime=date('Y-m-d H:i:s');
			$res[]=array('status_name'=>'User Confirmed Quotation','status_time'=>$ctime);
			$res=json_encode($res);
		...
		function emp_complete_inspection($input)
		{
			$this->db->select('status_history');
	        $this->db->from('tbl_service_request');
	        $this->db->where('sr_id',$input['sr_id']);
	        ...
	        $res=json_decode($res['status_history'],true);
			$ctime=date('Y-m-d H:i:s');			
			$res[]=array('status_name'=>'Employee Inspection Completed','status_time'=>$ctime);
			$res=json_encode($res);
		...
		function emp_get_payment($input)
		{
			$this->db->select('status_history');
	        $this->db->from('tbl_service_request');
	        $this->db->where('sr_id',$input['sr_id']);
	        ...
	        $res=json_decode($res['status_history'],true);
			$ctime=date('Y-m-d H:i:s');			
			$res[]=array('status_name'=>'Paid/Completed','status_time'=>$ctime);
			$res=json_encode($res);
			$this->db->set('status_history',$res);
			...
			$this->db->select('assigned_emp_id');
			$query=$this->db->get_where('tbl_service_request',array('sr_id'=>$input['sr_id']));
			$row=$query->row_array();
		...
		function update_sr_status($sr_id,$grand_total)
		{
			...
			$res=json_decode($res['status_history'],true);
			$ctime=date('Y-m-d H:i:s');
			$res[]=array('status_name'=>'Invoice Generated/Unpaid','status_time'=>$ctime);
			$res=json_encode($res);
		...
		{
			$this->db->select('status,status_history');
	        $this->db->from('tbl_service_request');
	        $this->db->where('sr_id',$data['sr_id']);
	        $query = $this->db->get();
	        $res=$query->row_array();
	        if($res['status']<9)
	        {
		        ...
		        if($res1['assigned_emp_id']!=0)
		        {
		        	$this->db->set('tbl_employee_status',0);
					$this->db->where('tbl_employee_id', $res1['assigned_emp_id']);
					$this->db->update('tbl_employee');
		        }

		        $res=json_decode($res['status_history'],true);
				$ctime=date('Y-m-d H:i:s');
				$res[]=array('status_name'=>'Cancelled by you.','status_time'=>$ctime);
				$res=json_encode($res);		        
		...
		function emp_track_location($data)
		{
			$value=array(
				'emp_track_latitude'=>$data['emp_track_latitude'],
				'emp_track_longitude'=>$data['emp_track_longitude'],
				'emp_track_place_name'=>$data['emp_track_place_name'],
				'tbl_employee_id'=>$data['tbl_employee_id'],
				'created_at'=>date('Y-m-d H:i:s')
				);
			$this->db->insert('tbl_emp_track', $value);
			return TRUE;            
		}

		function emp_get_location($data)
		{
			$this->db->where('tbl_employee_id',$data['tbl_employee_id']);
			$query = $this->db->get('tbl_emp_track');
			$rows = $query->result_array();
            return $rows;
		}

		function all_employee_location()
		{
			$sql="SELECT DISTINCT(t.tbl_employee_id),MAX(t.emp_track_id),t.emp_track_id,t.emp_track_latitude,t.emp_track_longitude,t.emp_track_place_name,t.created_at,e.tbl_employee_name,e.tbl_employee_image FROM tbl_emp_track t LEFT JOIN tbl_employee e ON e.tbl_employee_id=t.tbl_employee_id ";
			//echo $sql;die;
			$query = $this->db->query($sql);
			$row=$query->result_array();
			return $row;
		...
		public function employee_logout($data)
		{
			...
			if($res==0)
			{
				$this->db->set('tbl_users_platform','');
				$this->db->set('tbl_users_device_token','');
				$this->db->where('userId', $data['tbl_employee_id']);
				$this->db->where('tbl_users_device_id', $data['tbl_emp_device_id']);
				$this->db->update('tbl_users_devices');
			}
			//if($res)
				return 1;
		...
		function get_admin_order_list($status,$limit,$offset)
		{
			...
			if($status=='all')
			{
				$sql="SELECT sr.sr_id,sr.service_request_ref,sr.prefferd_date,sr.prefferd_time,sr.created_at,sr.status,t.time_slot_name,sr.pending_amount,sr.assigned_emp_id,sr.status,sr.tbl_user_id,sr.isQuotation,user.tbl_user_latitude,user.tbl_user_longitude,user.tbl_user_place,sr.pending_amount FROM tbl_service_request sr LEFT JOIN tbl_time_slot t ON t.time_slot_id=sr.prefferd_time LEFT JOIN tbl_user user ON user.tbl_user_id=sr.tbl_user_id  WHERE sr.isDeleted = 0 AND sr.status!=20 ORDER BY sr.sr_id DESC LIMIT ".$offset.",".$limit."";
			}
			else if($status=='inprogress')
			{
				$sql="SELECT sr.sr_id,sr.service_request_ref,sr.prefferd_date,sr.prefferd_time,sr.created_at,sr.status,t.time_slot_name,sr.pending_amount,sr.assigned_emp_id,sr.status,sr.tbl_user_id,sr.isQuotation,user.tbl_user_latitude,user.tbl_user_longitude,user.tbl_user_place,sr.pending_amount FROM tbl_service_request sr LEFT JOIN tbl_time_slot t ON t.time_slot_id=sr.prefferd_time LEFT JOIN tbl_user user ON user.tbl_user_id=sr.tbl_user_id  WHERE sr.isDeleted = 0 AND sr.status=4 OR sr.status=5 OR sr.status=6 OR sr.status=7 OR sr.status=8 OR sr.status=9 ORDER BY sr.sr_id DESC LIMIT ".$offset.",".$limit."";
			}
			else
			{
		...
		function get_notification_list($data)
		{
			...
			$offset=$offset-1;
			$offset=$offset*$limit;
			if($data['user_type']=='end_user')
			{
				$sql="SELECT user_notification_id,user_notification_sr_id,user_notification_message,user_notification_status,user_notification_createat FROM tbl_user_notification_log  WHERE isDeleted = 0 ORDER BY user_notification_id DESC LIMIT ".$offset.",".$limit."";
				$query = $this->db->query($sql);				
				$rows=$query->result_array();
				return $rows;
			}
			...
			{
				$sql="SELECT users_notification_id,users_notification_sr_id,users_notification_message,users_notification_status,users_notification_createat FROM tbl_users_notification_log  WHERE isDeleted = 0 ORDER BY users_notification_id DESC LIMIT ".$offset.",".$limit."";
				$query = $this->db->query($sql);				
				$rows=$query->result_array();
				return $rows;
			}			
		...
		function get_admin_order_detail($sr_id)
		{
			...
			$result=$query->result_array();
			$row['after_service_doc']=$result;
			$row['status_history']=json_decode($row['status_history']);
			//echo "<pre>";print_r($row);die;
		...
		public function admin_update_order_location($data)
		{			
			$this->db->set('tbl_user_latitude',$data['tbl_user_latitude']);
			$this->db->set('tbl_user_longitude',$data['tbl_user_longitude']);
			$this->db->set('tbl_user_place',$data['tbl_user_place']);
		...
		{
			$query=$this->db->get_where('tbl_employee',array('isDeleted'=>0));
			return $query->num_rows();
		}

		function get_available_emp_count()
		{
			$query=$this->db->get_where('tbl_employee',array('isDeleted'=>0,'tbl_employee_status'=>0));
			return $query->num_rows();
		}

		function get_busy_emp_count()
		{
			$query=$this->db->get_where('tbl_employee',array('isDeleted'=>0,'tbl_employee_status'=>1));
			return $query->num_rows();
		}

		function get_weekly_new_order_count()
		{
			$first_date=date('Y-m-d H:i:s');
			$second_date=date('Y-m-d H:i:s',(strtotime ('-7 day',strtotime($first_date))));
			$this->db->where('created_at >=', $second_date);
			$this->db->where('updated_at <=', $first_date);
			$this->db->where('isDeleted',0);
			$query=$this->db->get_where('tbl_service_request');
			return $query->num_rows();
		}

		function get_weekly_inprogress_order_count()
		{
			$first_date=date('Y-m-d H:i:s');
			$second_date=date('Y-m-d H:i:s',(strtotime ('-7 day',strtotime($first_date))));
			$this->db->where('created_at >=', $second_date);
		...
		function get_weekly_completed_order_count()
		{
			$first_date=date('Y-m-d H:i:s');
			$second_date=date('Y-m-d H:i:s',(strtotime ('-7 day',strtotime($first_date))));
			$this->db->where('created_at >=', $second_date);
		...
		function get_emp_list_order_by_status()
		{
			$this->db->select('tbl_employee_id,tbl_employee_name,tbl_employee_email,tbl_employee_mobile,tbl_employee_status,tbl_employee_image,tbl_employee_id_card,tbl_employee_skills,tbl_employee_basic_salary,tbl_employee_doj,tbl_employee_emegency_contact,tbl_employee_notes,tbl_employee_nationality,tbl_employee_latitude,tbl_employee_longitude,');
			$this->db->order_by('tbl_employee_status');
			$query=$this->db->get_where('tbl_employee',array('isDeleted'=>0));
			return $query->result_array();
		}
